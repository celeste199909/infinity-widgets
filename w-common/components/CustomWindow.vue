<template>
    <div :id="winId" class="custom-window flex flex-col overflow-hidden absolute select-none box-border m-0" :style="{
        ...
        <!-- bar -->
        <div class="w-full h-12 absolute top-0 left-0 right-0 text-slate-200 min-h-12 max-h-12 px-4 py-2 flex flex-row justify-between items-center z-10"
            @mousedown="startDrag">
            <!-- 名字 -->
            ...
            <div class="flex flex-row gap-3">
                <div @click="toggleFullscreen">
                    {{ windowState.isMaximized ? "还原" : "最大化" }}
                </div>
        ...
        <!-- 伸缩句柄 -->
        <!-- 右边 -->
        <div class="handle-r cursor-ew-resize absolute right-0 top-0 w-1 h-full" @mousedown="startResize"></div>
        <!-- 下边 -->
        <div class="handle-b cursor-ns-resize absolute bottom-0 left-0 right-0 w-full h-1" @mousedown="startResize"></div>
        <!-- 右下角 -->
        <div class="handle-rb cursor-nwse-resize absolute right-0 bottom-0 w-3 h-3 rounded-full opacity-20 hover:opacity-50 hover:w-5 hover:h-5 transition bg-blue-500 z-10"
